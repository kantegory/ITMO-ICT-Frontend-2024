    ...
    <main>
      <section id="restaurant-details">
        <div class="container">
          ...
          <div class="row">
            <div class="col-md-6">
              <img :src="'http://localhost:8081' + restaurant.image" class="card-img-top" :alt="restaurant.name">
            </div>
            <div class="col-md-6">
          ...
          <div class="modal-body">
            <form @submit.prevent="submitBooking">
              <div class="mb-3">
                ...
                <input type="date" v-model="booking.date" class="form-control" required :min="minDate">
              </div>
              <div class="mb-3">
...
export default {
  ...
  setup() {
    ...
    const loadRestaurant = async () => {
      try {
        const data = await api.getRestaurantById(route.params.id);
    ...
    const closeBookingModal = () => {
      if (bookingModal.value) {
        const modalInstance = Modal.getInstance(bookingModal.value);
        if (modalInstance) {
          modalInstance.hide();
        }
      }
    ...
    const submitBooking = async () => {
      ...
      try {
        ...
        errorMessage.value = 'Ошибка при бронировании. Попробуйте еще раз.';
      }
    };
